...
#import "UIApplication+PS_.h"
#import "UIDevice+PS_.h"
#import <sys/sysctl.h>
#import <mach/mach.h>
...
@interface _PSUIApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation _PSUIApplicationNetworkIndicatorInfo
@end

@implementation UIApplication (PS_)

- (NSURL *)ps_documentsURL {
    return [[[NSFileManager defaultManager]
             URLsForDirectory:NSDocumentDirectory
             inDomains:NSUserDomainMask] lastObject];
}

- (NSString *)ps_documentsPath {
...
- (NSString *)ps_libraryPath {
    return [NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) firstObject];
}

- (BOOL)ps_isPirated {
    
    if ([[UIDevice currentDevice] ps_isSimulator]) {
        return YES; //  是不是盗版
    }
    ...
    if (![self _ps_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _ps_fileExistInMainBundle:@"SC_Info"]) {
        return YES;
    }
...
- (BOOL)_ps_fileExistInMainBundle:(NSString *)name {
    NSString *bundlePath = [[NSBundle mainBundle] bundlePath];
    NSString *path = [NSString stringWithFormat:@"%@/%@", bundlePath, name];
    return [[NSFileManager defaultManager] fileExistsAtPath:path];
}

- (NSString *)ps_appBundleName {
    return [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleName"];
}

- (NSString *)ps_appBundleID {
...
- (NSString *)ps_appBuildVersion {
    return [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleVersion"];
}

- (BOOL)ps_isBeingDebugged {
...
- (_PSUIApplicationNetworkIndicatorInfo *) networkActivityInfo {
    return objc_getAssociatedObject(self, @selector(setNetworkActivityInfo:));
}

- (void) setNetworkActivityInfo:(_PSUIApplicationNetworkIndicatorInfo *) info {
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _PSUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_PSUIApplicationNetworkIndicatorInfo new];
...
+ (UIApplication *)ps_sharedExtensionApplication {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wundeclared-selector"
    return [self ps_isAppExtension] ? nil : [UIApplication performSelector:@selector(sharedApplication)];
