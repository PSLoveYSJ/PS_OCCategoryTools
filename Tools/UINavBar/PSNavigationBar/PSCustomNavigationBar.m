...
#import "PSCustomNavigationBar.h"
...
@implementation UIViewController (PSRoute)

- (void)ps_toLastViewController
{
...
+ (UIViewController*)ps_currentViewController {
    UIViewController* rootViewController = [UIApplication sharedApplication].delegate.window.rootViewController;
    return [self ps_currentViewControllerFrom:rootViewController];
}

+ (UIViewController*)ps_currentViewControllerFrom:(UIViewController*)viewController
{
    if ([viewController isKindOfClass:[UINavigationController class]]) {
        UINavigationController* navigationController = (UINavigationController *)viewController;
        return [self ps_currentViewControllerFrom:navigationController.viewControllers.lastObject];
    }
    else if([viewController isKindOfClass:[UITabBarController class]]) {
        UITabBarController* tabBarController = (UITabBarController *)viewController;
        return [self ps_currentViewControllerFrom:tabBarController.selectedViewController];
    }
    else if (viewController.presentedViewController != nil) {
        return [self ps_currentViewControllerFrom:viewController.presentedViewController];
    }
    else {
...
@interface PSCustomNavigationBar ()
...
@implementation PSCustomNavigationBar

+ (instancetype)CustomNavigationBar {
    PSCustomNavigationBar *navigationBar = [[self alloc] initWithFrame:CGRectMake(0, 0, kWRScreenWidth, [PSCustomNavigationBar navBarBottom])];
...
-(void)updateFrame {
    NSInteger top = ([PSCustomNavigationBar isIphoneX]) ? 44 : 20;
    NSInteger margin = 0;
...
-(void)clickBack {
    if (self.onClickLeftButton) {
        ...
        UIViewController *currentVC = [UIViewController ps_currentViewController];
        [currentVC ps_toLastViewController];
    }
}
-(void)clickRight {
...
- (void)ps_setLeftButtonWithImage:(UIImage *)image title:(NSString *)title titleColor:(UIColor *)titleColor {
    [self ps_setLeftButtonWithNormal:image highlighted:image title:title titleColor:titleColor];
}
- (void)ps_setLeftButtonWithNormal:(UIImage *)normal highlighted:(UIImage *)highlighted {
    [self ps_setLeftButtonWithNormal:normal highlighted:highlighted title:nil titleColor:nil];
}
- (void)ps_setLeftButtonWithImage:(UIImage *)image {
    [self ps_setLeftButtonWithNormal:image highlighted:image title:nil titleColor:nil];
}
- (void)ps_setLeftButtonWithTitle:(NSString *)title titleColor:(UIColor *)titleColor {
    [self ps_setLeftButtonWithNormal:nil highlighted:nil title:title titleColor:titleColor];
}
...
- (void)ps_setRightButtonWithImage:(UIImage *)image title:(NSString *)title titleColor:(UIColor *)titleColor {
    [self ps_setRightButtonWithNormal:image highlighted:image title:title titleColor:titleColor];
}
- (void)ps_setRightButtonWithNormal:(UIImage *)normal highlighted:(UIImage *)highlighted {
    [self ps_setRightButtonWithNormal:normal highlighted:highlighted title:nil titleColor:nil];
}
- (void)ps_setRightButtonWithImage:(UIImage *)image {
    [self ps_setRightButtonWithNormal:image highlighted:image title:nil titleColor:nil];
}
- (void)ps_setRightButtonWithTitle:(NSString *)title titleColor:(UIColor *)titleColor {
    [self ps_setRightButtonWithNormal:nil highlighted:nil title:title titleColor:titleColor];
}
