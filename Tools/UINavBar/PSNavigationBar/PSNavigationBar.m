...
//=============================================================================
@implementation PSNavigationBar (PSDefault)
...
//=============================================================================
@implementation UINavigationBar (PSAddition)
...
- (void)setBackgroundView:(UIView *)backgroundView {
    if (backgroundView) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ps_keyboardDidShow) name:UIKeyboardDidShowNotification object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ps_keyboardWillHide) name:UIKeyboardWillHideNotification object:nil];
    } else {
...
- (void)ps_setBackgroundImage:(UIImage *)image {
    ...
    if (self.backgroundImageView == nil) {
        ...
        if (self.subviews.count > 0) {
            self.backgroundImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.bounds), [PSNavigationBar navBarBottom])];
...
- (void)ps_setBackgroundColor:(UIColor *)color {
    ...
    if (self.backgroundView == nil) {
        // add a image(nil color) to _UIBarBackground make it clear
        [self setBackgroundImage:[UIImage new] forBarMetrics:UIBarMetricsDefault];
        self.backgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.bounds), [PSNavigationBar navBarBottom])];
...
- (void)ps_keyboardDidShow {
    [self ps_restoreUIBarBackgroundFrame];
}
- (void)ps_keyboardWillHide {
    [self ps_restoreUIBarBackgroundFrame];
}
- (void)ps_restoreUIBarBackgroundFrame {
    ...
    for (UIView *view in self.subviews) {
        ...
        if (_UIBarBackgroundClass != nil) {
            if ([view isKindOfClass:_UIBarBackgroundClass]) {
                view.frame = CGRectMake(0, self.frame.size.height-[PSNavigationBar navBarBottom], [PSNavigationBar screenWidth], [PSNavigationBar navBarBottom]);
            }
        }
    }
}
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int i = 0; i < 1;  i++) {
            ...
            NSString *newSelectorStr = [NSString stringWithFormat:@"ps_%@", NSStringFromSelector(selector)];
...
- (void)ps_setTitleTextAttributes:(NSDictionary<NSString *,id> *)titleTextAttributes {
    ...
    if (originTitleTextAttributes == nil) {
        [self ps_setTitleTextAttributes:newTitleTextAttributes];
        return;
    }
...
//==========================================================================
@implementation UINavigationController (PSAddition)
...
- (void)updateNavigationBarWithFromVC:(UIViewController *)fromVC toVC:(UIViewController *)toVC progress:(CGFloat)progress {
    ...
    UIColor *fromBarTintColor = [fromVC ps_navBarBarTintColor];
    UIColor *toBarTintColor = [toVC ps_navBarBarTintColor];
    UIColor *newBarTintColor = [PSNavigationBar middleColor:fromBarTintColor toColor:toBarTintColor percent:progress];
    if ([PSNavigationBar needUpdateNavigationBar:fromVC] || [PSNavigationBar needUpdateNavigationBar:toVC]) {
        [self setNeedsNavigationBarUpdateForBarTintColor:newBarTintColor];
    }
    
    // change navBarTintColor
    UIColor *fromTintColor = [fromVC ps_navBarTintColor];
    UIColor *toTintColor = [toVC ps_navBarTintColor];
    UIColor *newTintColor = [PSNavigationBar middleColor:fromTintColor toColor:toTintColor percent:progress];
    if ([PSNavigationBar needUpdateNavigationBar:fromVC]) {
        [self setNeedsNavigationBarUpdateForTintColor:newTintColor];
    }

    // change navBarTitleColor（在ps_popToViewController:animated:方法中直接改变标题颜色）
    UIColor *fromTitleColor = [fromVC ps_navBarTitleColor];
    UIColor *toTitleColor = [toVC ps_navBarTitleColor];
    UIColor *newTitleColor = [PSNavigationBar middleColor:fromTitleColor toColor:toTitleColor percent:progress];
    [self setNeedsNavigationBarUpdateForTitleColor:newTitleColor];
    
    // change navBar _UIBarBackground alpha
    CGFloat fromBarBackgroundAlpha = [fromVC ps_navBarBackgroundAlpha];
    CGFloat toBarBackgroundAlpha = [toVC ps_navBarBackgroundAlpha];
    CGFloat newBarBackgroundAlpha = [PSNavigationBar middleAlpha:fromBarBackgroundAlpha toAlpha:toBarBackgroundAlpha percent:progress];
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int i = 0; i < 4;  i++) {
            SEL selector = needSwizzleSelectors[i];
            NSString *newSelectorStr = [[NSString stringWithFormat:@"ps_%@", NSStringFromSelector(selector)] stringByReplacingOccurrencesOfString:@"__" withString:@"_"];
            Method originMethod = class_getInstanceMethod(self, selector);
            Method swizzledMethod = class_getInstanceMethod(self, NSSelectorFromString(newSelectorStr));
            method_exchangeImplementations(originMethod, swizzledMethod);
        }
...
- (NSArray<UIViewController *> *)ps_popToViewController:(UIViewController *)viewController animated:(BOOL)animated {
    // pop 的时候直接改变 barTintColor、tintColor
    [self setNeedsNavigationBarUpdateForTitleColor:[viewController ps_navBarTitleColor]];
    [self setNeedsNavigationBarUpdateForTintColor:[viewController ps_navBarTintColor]];
    ...
    [CATransaction begin];
    NSArray<UIViewController *> *vcs = [self ps_popToViewController:viewController animated:animated];
    [CATransaction commit];
    return vcs;
}

- (NSArray<UIViewController *> *)ps_popToRootViewControllerAnimated:(BOOL)animated {
    __block CADisplayLink *displayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(popNeedDisplay)];
    [displayLink addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];
    [CATransaction setCompletionBlock:^{
        [displayLink invalidate];
    ...
    [CATransaction setAnimationDuration:wrPopDuration];
    [CATransaction begin];
    NSArray<UIViewController *> *vcs = [self ps_popToRootViewControllerAnimated:animated];
    [CATransaction commit];
    return vcs;
}
...
- (void)dealInteractionChanges:(id<UIViewControllerTransitionCoordinatorContext>)context {
    void (^animations) (UITransitionContextViewControllerKey) = ^(UITransitionContextViewControllerKey key){
        ...
        UIColor *curColor = [vc ps_navBarBarTintColor];
        CGFloat curAlpha = [vc ps_navBarBackgroundAlpha];
...
- (void)ps_updateInteractiveTransition:(CGFloat)percentComplete {
    UIViewController *fromVC = [self.topViewController.transitionCoordinator viewControllerForKey:UITransitionContextFromViewControllerKey];
    UIViewController *toVC = [self.topViewController.transitionCoordinator viewControllerForKey:UITransitionContextToViewControllerKey];
    [self updateNavigationBarWithFromVC:fromVC toVC:toVC progress:percentComplete];
    [self ps_updateInteractiveTransition:percentComplete];
}
...
//==========================================================================
@implementation UIViewController (PSAddition)
...
- (UIImage *)ps_navBarBackgroundImage {
    ...
    image = (image == nil) ? [PSNavigationBar defaultNavBarBackgroundImage] : image;
    return image;
}
- (void)ps_setNavBarBackgroundImage:(UIImage *)image {
    if ([[self ps_customNavBar] isKindOfClass:[UINavigationBar class]]) {
    //  UINavigationBar *navBar = (UINavigationBar *)[self ps_customNavBar];
    //  [navBar ps_setBackgroundImage:image];
    } else {
...
- (UIColor *)ps_systemNavBarBarTintColor {
    return (UIColor *)objc_getAssociatedObject(self, &kWRSystemNavBarBarTintColorKey);
}
- (void)ps_setSystemNavBarBarTintColor:(UIColor *)color {
...
- (UIColor *)ps_navBarBarTintColor {
    ...
    if (![PSNavigationBar needUpdateNavigationBar:self]) {
        if ([self ps_systemNavBarBarTintColor] == nil) {
            barTintColor = self.navigationController.navigationBar.barTintColor;
        } else {
            barTintColor = [self ps_systemNavBarBarTintColor];
        }
    }
    return (barTintColor != nil) ? barTintColor : [PSNavigationBar defaultNavBarBarTintColor];
}
- (void)ps_setNavBarBarTintColor:(UIColor *)color {
    ...
    if ([[self ps_customNavBar] isKindOfClass:[UINavigationBar class]]) {
    //  UINavigationBar *navBar = (UINavigationBar *)[self ps_customNavBar];
    //  [navBar ps_setBackgroundColor:color];
    } else {
...
- (CGFloat)ps_navBarBackgroundAlpha {
    ...
    return (barBackgroundAlpha != nil) ? [barBackgroundAlpha floatValue] : [PSNavigationBar defaultNavBarBackgroundAlpha];
}
- (void)ps_setNavBarBackgroundAlpha:(CGFloat)alpha {
    ...
    if ([[self ps_customNavBar] isKindOfClass:[UINavigationBar class]]) {
    //  UINavigationBar *navBar = (UINavigationBar *)[self ps_customNavBar];
    //  [navBar ps_setBackgroundAlpha:alpha];
    } else {
        BOOL isRootViewController = (self.navigationController.viewControllers.firstObject == self);
        if (([self pushToCurrentVCFinished] == YES || isRootViewController == YES) && [self pushToNextVCFinished] == NO) {
...
- (UIColor *)ps_systemNavBarTintColor {
    return (UIColor *)objc_getAssociatedObject(self, &kWRSystemNavBarTintColorKey);
}
- (void)ps_setSystemNavBarTintColor:(UIColor *)color {
...
- (UIColor *)ps_navBarTintColor {
    ...
    if (![PSNavigationBar needUpdateNavigationBar:self]) {
        if ([self ps_systemNavBarTintColor] == nil) {
            tintColor = self.navigationController.navigationBar.tintColor;
        } else {
            tintColor = [self ps_systemNavBarTintColor];
        }
    }
    return (tintColor != nil) ? tintColor : [PSNavigationBar defaultNavBarTintColor];
}
...
- (UIColor *)ps_systemNavBarTitleColor {
    return (UIColor *)objc_getAssociatedObject(self, &kWRSystemNavBarTitleColorKey);
}
- (void)ps_setSystemNavBarTitleColor:(UIColor *)color {
...
- (UIColor *)ps_navBarTitleColor {
    ...
    if (![PSNavigationBar needUpdateNavigationBar:self]) {
        if ([self ps_systemNavBarTitleColor] == nil) {
            titleColor = self.navigationController.navigationBar.titleTextAttributes[@"NSColor"];
        } else {
            titleColor = [self ps_systemNavBarTitleColor];
        }
    }
    return (titleColor != nil) ? titleColor : [PSNavigationBar defaultNavBarTitleColor];
}
...
- (UIStatusBarStyle)ps_statusBarStyle {
    ...
    return (style != nil) ? [style integerValue] : [PSNavigationBar defaultStatusBarStyle];
}
...
- (BOOL)ps_navBarShadowImageHidden {
    ...
    return (hidden != nil) ? [hidden boolValue] : [PSNavigationBar defaultNavBarShadowImageHidden];
}

// custom navigationBar
- (UIView *)ps_customNavBar {
    UIView *navBar = objc_getAssociatedObject(self, &kWRCustomNavBarKey);
    return (navBar != nil) ? navBar : [UIView new];
}
- (void)ps_setCustomNavBar:(UINavigationBar *)navBar {
...
- (void)ps_viewWillAppear:(BOOL)animated {
    if ([self canUpdateNavigationBar]) {
        if (![PSNavigationBar needUpdateNavigationBar:self]) {
            if ([self ps_systemNavBarBarTintColor] == nil) {
                [self ps_setSystemNavBarBarTintColor:[self ps_navBarBarTintColor]];
            }
            if ([self ps_systemNavBarTintColor] == nil) {
                [self ps_setSystemNavBarTintColor:[self ps_navBarTintColor]];
            }
            if ([self ps_systemNavBarTitleColor] == nil) {
                [self ps_setSystemNavBarTitleColor:[self ps_navBarTitleColor]];
            }
            [self.navigationController setNeedsNavigationBarUpdateForTintColor:[self ps_navBarTintColor]];
        }
        [self setPushToNextVCFinished:NO];
        [self.navigationController setNeedsNavigationBarUpdateForTitleColor:[self ps_navBarTitleColor]];
    }
...
- (void)ps_viewDidAppear:(BOOL)animated
{
    ...
    if ([self canUpdateNavigationBar] == YES)
    {
        UIImage *barBgImage = [self ps_navBarBackgroundImage];
        if (barBgImage != nil) {
            [self.navigationController setNeedsNavigationBarUpdateForBarBackgroundImage:barBgImage];
        } else {
            if ([PSNavigationBar needUpdateNavigationBar:self]) {
                [self.navigationController setNeedsNavigationBarUpdateForBarTintColor:[self ps_navBarBarTintColor]];
            }
        }
        [self.navigationController setNeedsNavigationBarUpdateForBarBackgroundAlpha:[self ps_navBarBackgroundAlpha]];
        [self.navigationController setNeedsNavigationBarUpdateForTintColor:[self ps_navBarTintColor]];
        [self.navigationController setNeedsNavigationBarUpdateForTitleColor:[self ps_navBarTitleColor]];
        [self.navigationController setNeedsNavigationBarUpdateForShadowImageHidden:[self ps_navBarShadowImageHidden]];
    }
    [self ps_viewDidAppear:animated];
}

- (void)ps_viewDidDisappear:(BOOL)animated {
    if (![PSNavigationBar needUpdateNavigationBar:self] && !self.navigationController) {
        [self ps_setSystemNavBarBarTintColor:nil];
        [self ps_setSystemNavBarTintColor:nil];
        [self ps_setSystemNavBarTitleColor:nil];
    }
    [self ps_viewDidDisappear:animated];
}
