...
@implementation PSThreadSafeArray
{
...
- (BOOL)isEqualToArray:(NSArray *)otherArray {
    ...
    if ([otherArray isKindOfClass:PSThreadSafeArray.class]) {
        PSThreadSafeArray *other = (id)otherArray;
        BOOL isEqual;
...
- (BOOL)isEqual:(id)object {
    ...
    if ([object isKindOfClass:PSThreadSafeArray.class]) {
        PSThreadSafeArray *other = object;
        BOOL isEqual;
        dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER);
        dispatch_semaphore_wait(other->_lock, DISPATCH_TIME_FOREVER);
...
#pragma mark - custom methods for NSArray(PSAdd)

- (id)randomObject {
    LOCK(id o = [_arr ps_randomObject]) return o;
}

- (id)objectOrNilAtIndex:(NSUInteger)index {
    LOCK(id o = [_arr ps_objectOrNilAtIndex:index]) return o;
}

- (void)removeFirstObject {
    LOCK([_arr ps_removeFirstObject]);
}

- (id)popFirstObject {
    LOCK(id o = [_arr ps_popFirstObject]) return o;
}

- (id)popLastObject {
    LOCK(id o = [_arr ps_popLastObject]) return o;
}

- (void)appendObjects:(NSArray *)objects {
    LOCK([_arr ps_appendObjects:objects]);
}

- (void)prependObjects:(NSArray *)objects {
    LOCK([_arr ps_prependObjects:objects]);
}

- (void)insertObjects:(NSArray *)objects atIndex:(NSUInteger)index {
    LOCK([_arr ps_insertObjects:objects atIndex:index]);
}

- (void)reverse {
    LOCK([_arr ps_reverse]);
}

- (void)shuffle {
    LOCK([_arr ps_shuffle]);
}
